...
void interleaveQueue(Queue *q)
{
	...
	int size = q->ll.size / 2; 
	// push first half, will be in reversed order in stack
	...
	// move second half back to the end
	for (int i = 0; i < size; i++) {
		int val = dequeue(q);
		enqueue(q, val); 
	}

	// move reversed first half to stack, will result in nonreversed order in stack
	for (int i = 0; i < size; i++) {
		push(&s, q->ll.head->item);
		dequeue(q); 
	}

	// individually enqueu from stack then move the head to the end
	for (int i = 0; i < size; i++) {
		enqueue(q, s.ll.head->item); 
		pop(&s); 
