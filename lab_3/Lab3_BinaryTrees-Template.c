...
int smallestValue(BTNode *node) {
	...
	if (node->left == NULL) {
		...
		if (node->item < r) return node->item; 
		else return r;

	} 
	else if (node->right == NULL) {
		l = smallestValue(node->left); 
		if (node->item < l) return node->item; 
		else return l; 
	}
	else {
		l = smallestValue(node->left); 
		...
		if (node->item < l & node->item < r) return node->item; 
		else if (l < node->item & l < r) return l; 
		else return r; 
	}
